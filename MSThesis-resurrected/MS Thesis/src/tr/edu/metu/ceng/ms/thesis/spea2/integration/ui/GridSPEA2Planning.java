package tr.edu.metu.ceng.ms.thesis.spea2.integration.ui;
...
import tr.edu.metu.ceng.ms.thesis.spea2.integration.core.SPEA2PathPlanner;
...
public class GridSPEA2Planning {
	...
	private MOSPEA2GeneticMap gmap;
	...
	// private static final String EXECUTION_FILE =
	// "/experimental/journal-tests/partiallyobservable/120x120/120x120_60(46)";
	private static final String EXECUTION_FILE = "/experimental/journal-tests/partiallyobservable/120x120/120x120_40(38)";
	...
	public static void main(String[] args) throws InterruptedException {
		...
		GridSPEA2Planning mogpPlanning = new GridSPEA2Planning();
	...
	public void execute() throws InterruptedException {
		...
		gmap = mapGen.generateSPEA2Map();

		System.out.println(EXECUTION_FILE.substring(EXECUTION_FILE.lastIndexOf(File.separator)+1));
		
		Rectangle2D mapBounds = new Rectangle2D.Double(0.0, 0.0, gmap.size(0),
				gmap.size(1));
		// initialize Multi-Objective Genetic Path Planner.
		SPEA2PathPlanner spea2 = new SPEA2PathPlanner(gmap, new IntCoord(
				gmap.getStart()), new IntCoord(gmap.getTmpGoal()));
		...
		while (true) {
			if (needToReplan.getAndSet(false)) {
				synchronized (spea2) {
					List<Path<Coordinate>> paths = spea2.plan();
					...
					if (paths.size() == 0
							&& !Arrays
									.equals(gmap.getTmpGoal(), gmap.getGoal())) {

						IntCoord tmpGoal = gmap.updateTmpGoal();
						spea2.updateGoal(tmpGoal);
						prevTmpGoal = tmpGoal;
					...
					if (paths.size() > 0) {
						if (!Arrays.equals(gmap.getCurrentAgentLocation()
							...
							operateOnPath(index, paths, spea2, mp, 1);
						} else {
	...
	private void operateOnPath(int pathNumber, List<Path<Coordinate>> paths,
			SPEA2PathPlanner spea2Planner, MapPanel mp, int index)
		...
		try {
			...
			System.out.println("###Total Execution Time: "
					+ SPEA2PathPlanner.getTotalExecTime());
			System.out.println("###Actual Path Cost: " + actualPathCost.toShortenString());
			return;
		}
		...
		if (tmpGoal.equals(prevTmpGoal)) {
			// Thread.sleep(500);
			operateOnPath(pathNumber, paths, spea2Planner, mp, index + 1);
		} else {
			gmap.clearBackpointers();
			spea2Planner.updateGoal(tmpGoal);
			spea2Planner.updateStart(newAgentLoc);
			prevTmpGoal = tmpGoal;
			mp.setShape(
					"TmpGoal",
					dot,
	...
	public class VisualizedMapPanel extends MapPanel {
		...
		private SPEA2PathPlanner spea2;

		private Rectangle2D mapBounds;

		private MOStaticMap sm;

		public VisualizedMapPanel(SPEA2PathPlanner mogpp, MOStaticMap sm,
