...
package tr.edu.metu.ceng.ms.thesis.spea2.util;
...
import tr.edu.metu.ceng.ms.thesis.spea2.util.comparators.DominanceComparator;
import tr.edu.metu.ceng.ms.thesis.spea2.util.comparators.FitnessComparator;
...
public class Spea2Fitness {
	 ...
	 */
	private SPEA2SolutionSet solutionSet_ = null;
	 ...
	 */
	public Spea2Fitness(SPEA2SolutionSet solutionSet) {
	 ...
	 */
	public SPEA2SolutionSet environmentalSelection(int size) {
		...
		SPEA2SolutionSet aux = new SPEA2SolutionSet(solutionSet_.size());
		...
		for (int pos = 0; pos < aux.size(); pos++) {
			...
			for (int ref = 0; ref < aux.size(); ref++) {
				if (pos != ref) {
					distanceNodeList.add(new DistanceNode(distance[pos][ref],
							ref));
				} // if
			} // for
		...
		while (aux.size() > size) {
			...
			for (List<DistanceNode> dn : distanceList) {
				if (dn.get(0).getDistance() < minDistance) {
					...
					int k = 0;
					while ((dn.get(k).getDistance() == distanceList
							.get(toRemove).get(k).getDistance())
							&& k < (distanceList.get(i).size() - 1)) {
						k++;
					}

					if (dn.get(k).getDistance() < distanceList.get(toRemove)
							.get(k).getDistance()) {
						toRemove = i;
					} // if
				} // if
