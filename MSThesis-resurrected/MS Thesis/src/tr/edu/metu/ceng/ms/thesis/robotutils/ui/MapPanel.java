...
public class MapPanel extends JPanel {
	...
	private static final Color PATH_COLOR = new Color(123, 104, 238);
	private static final long serialVersionUID = -2119904211158385572L;

	private final LinkedHashMap<String, MapIcon> _icons = new LinkedHashMap<String, MapIcon>();
	private final AffineTransform _mapTransform = new AffineTransform();

	private final MapKeyboardListener _mkl = new MapKeyboardListener();

	// mouse and key listeners for this map panel.
	private final MapMouseListener _mt = new MapMouseListener();
	private final LinkedHashMap<String, MapShape> _shapes = new LinkedHashMap<String, MapShape>();

	private boolean ctrlIsMasked = Boolean.FALSE;
	...
	public void paintComponent(Graphics g) {
		...
		if (downPoint != null && currPoint != null) {
			g2.translate(currPoint.getX() - downPoint.getX(), currPoint.getY()
					- downPoint.getY());
		}
	...
	public void setCtrlIsMasked(boolean state) {
		this.ctrlIsMasked = state;
	}
	 ...
	 * @param xform
	 *            a transform from map coordinates to image coordinates
	 */
	 ...
	 *            an image for the icon
	 * @param bounds
	 *            a box specifying the map coordinates of the icon's edges
	 */
	...
	/**
	 * @see MapPanel#setShape(java.lang.String, java.awt.Shape,
	 *      java.awt.geom.AffineTransform, java.awt.Color, java.awt.Stroke)
	 * 
	 * @param name
	 *            a name for the shape
	 * @param shape
	 *            an object represeting the shape to be drawn
	 * @param xform
	 *            the transformation of the shape to map coordinates
	 */
	 ...
	 *            the color with which to draw the shape
	 * @param stroke
	 *            the stroke width to use when drawing the shape
	 */
	public void setShape(String name, Shape shape, AffineTransform xform,
	 ...
	 * 
	 * @param left
	 *            the map coordinate of the left side of the window
	 * @param right
	 *            the map coordinate of the right side of the window
	 * @param top
	 *            the map coordinate of the top of the window
	 * @param bottom
	 *            the map coordination of the bottom of the window
	 */
	 ...
	 * 
	 * @param bounds
	 *            A rectangle representing the boundary of the window
	 */
	public void setView(Rectangle2D bounds) {
		...
		_mapTransform.scale(this.getWidth() / bounds.getWidth(),
	 ...
	 * 
	 * @param x
	 *            The x-position (left-right) in the map frame.
	 * @param y
	 *            The y-position (up-down) in the map frame.
	 */
	...
	private class MapMouseListener implements MouseInputListener,
			MouseWheelListener {
		...
		public void mouseClicked(MouseEvent evt) {
			...
			onClick(map.getX(), map.getY(), evt.getButton(),
					evt.getClickCount());
		}
		...
		public void mouseReleased(MouseEvent e) {
			...
			_mapTransform.preConcatenate(AffineTransform.getTranslateInstance(
					dx, dy));
			downPoint = null;
			MapPanel.this.repaint();
		}
